...
const ViewLoan = () => {
  ...
  const acknowledgeMissingDocProvided = () => {
    axios
      .post(
        `${baseUrl}/loan/${loanId}/doc/re-assessment`,
        {},
        {
          headers: {
            "user-id": user,
  ...
  const acknowledgeCorrection = () => {
    axios
      .post(`${baseUrl}/loan/${loanId}/doc/update-details`, {
        headers: {
          "user-id": user,
        },
      })
  ...
  const approveOrRejectLoanRequest = (action: string) => {
    axios
      .post(
        `${baseUrl}/loan/${loanId}/action/${action}`,
        { comments: "test" },
        {
          headers: {
            "user-id": user,
  ...
  const cancelLoan = () => {
    axios
      .delete(`${baseUrl}/loan/${loanId}/action/CANCEL`, {
        headers: {
          "user-id": user,
        },
      })
    ...
    <div>
      <Grid>
        <GridRow columns={2}>
          ...
          <GridColumn width={4}>
            <Link className="width_100" to={"/dashboard"}>
      ...
      <div className="viewloan">
        {loanSummary && (
          <LoanInfo
            ...
            amount={loanSummary?.amount?.toLocaleString()}
            loanCategory={loanSummary.loanType}
          ...
          <GridRow columns={5}>
            <GridColumn>
              {loanSummary?.possibleActivities?.includes("CAN_CANCEL") && (
                <Button primary className="width_100" onClick={cancelLoan}>
                  Cancel Loan
                </Button>
              )}
            </GridColumn>
            ...
            <GridColumn></GridColumn>
            <GridColumn>
              {loanSummary?.possibleActivities?.includes(
                "CAN_APPROVEORREJECT"
              ) && (
                <Button
                  primary
                  className="width_100"
                  onClick={() => approveOrRejectLoanRequest("REJECT")}
                >
                  Reject
                </Button>
              )}
            ...
            <GridColumn>
              {loanSummary?.possibleActivities?.includes(
                ...
                <Button
                  primary
                  className="width_100"
                  onClick={() => approveOrRejectLoanRequest("APPROVE")}
                >
                  Approve
                </Button>
              )}
              ...
              {loanSummary?.possibleActivities?.includes(
                "CAN_ACKNOWLEDGE_CORRECTION"
              ) && (
                <Button
                  primary
                  className="width_100"
                  onClick={acknowledgeCorrection}
                >
                  Acknowledge Correction
                </Button>
              )}
              {loanSummary?.possibleActivities?.includes(
                "CAN_ACKNOWLEDGE_DOC_PROVIDED"
              ) && (
                <Button
                  primary
                  className="width_100"
                  onClick={acknowledgeMissingDocProvided}
                >
                  {" "}
                  I have provided missing document
                </Button>
              )}
            </GridColumn>
          </GridRow>
